...
[ApiController]
[Route("api/loandetails-report")]
public class LoanDetailsReportController : ControllerBase
{
    private readonly JlmsDbContext _db;

    public LoanDetailsReportController(JlmsDbContext db)
    {
        _db = db;
    }
    ...
    [HttpGet]
    public async Task<ActionResult<LoanDetailsReportPagedDto>> Get(
        ...
        [FromQuery] int pageSize = 25)
    {
     ...
     .ToListAsync();
        ...
        var allRows = loans.Select(l =>
        {
            ...
            var modelNames = l.JewelItems.Select(ji => ji.Model ?? "").Where(m => m != "").Distinct();
            var variantNames = l.JewelItems.Select(ji => ji.Varient ?? "").Where(v => v != "").Distinct();
            // ^ adjust property name to whatever your JewelItem entity uses (Image 3 shows the DB column as "Varient")

            return new LoanDetailsReportRowDto(
                LoanId: l.LoanId,
                ...
                Quantity: l.JewelItems.Sum(ji => ji.Quantity),
                Model: string.Join(", ", modelNames),
                Variant: string.Join(", ", variantNames),
        ...
        var result = new LoanDetailsReportPagedDto(
     Items: pagedItems,
